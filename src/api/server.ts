...
import {
  featureManager,
  type FeatureName,
} from "../features/featureManager.js";
...
const generateAdminSessionToken = (
  ...
  console.log(
    `🔑 Created admin session token for user ${userId} on guild ${guildId} (Level ${adminLevel})`,
  );
...
let registerGuildCommandsFunction: ((guildId: string) => Promise<void>) | null =
  null;

export function setRegisterGuildCommandsFunction(
    ...
    try {
      ...
      if (storedToken && storedToken.jwt_token === token) {
        ...
        try {
          ...
          let guildsResponse = await fetch(
          ...
          if (userResponse.status === 401 && refreshToken) {
            ...
            if (refreshResult) {
              ...
              await dbManager.updateUserTokens(
                decoded.userId,
                accessToken,
...
const requireGuildAccess = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  ...
  if (adminStatus.isAdmin) {
    console.log(
      `✅ Global admin ${user.username || user.id} (Level ${adminStatus.level}) accessing guild ${guildId} - bypassing guild permission check`,
    );
  ...
  console.log(
    `✅ User ${user.username || user.id} has MANAGE_SERVER permission for guild ${guildId}`,
  );
  next();
};

// Middleware to require global admin access
const requireGlobalAdmin = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  ...
  console.log(
    `🔐 Global admin ${user.username} (Level ${adminStatus.level}) accessing admin endpoint`,
  );
  next();
};
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.log(
        `🔑 Generated admin session token for ${user.username} (${user.id}) on guild ${guildId}`,
      );
      ...
      return res
        .status(500)
        .json({ error: "Failed to generate admin session" });
    }
  },
);
...
app.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await dbManager.setGlobalSetting(
        key,
        value,
        type || "string",
        description || "",
      ...
      await dbManager.logAdminActivity(
        user.id,
...
app.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const adminId = await dbManager.addGlobalAdmin(
        userId,
        username,
        level || 1,
        user.id,
      ...
      await dbManager.logAdminActivity(
        ...
        "user",
        userId,
        `Added ${username} as admin (Level ${level || 1})`,
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (userId === user.id) {
        return res
          .status(400)
          .json({ error: "Cannot remove yourself as admin" });
      }
...
app.get(
  ...
  async (req: Request, res: Response) => {
    ...
    const token = jwtSign({ userId: user.id }, JWT_SECRET, {
      expiresIn: "24h",
    });
    ...
    try {
      ...
      console.log(
        `✅ Stored JWT token for user ${user.id} with Discord tokens`,
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!guild) {
        return res
          .status(404)
          .json({ error: "Guild not found or bot not in guild" });
      }
      ...
      const stats = {
        ...
        botMembers: guild.members.cache.filter((member) => member.user.bot)
          .size,
        humanMembers: guild.members.cache.filter((member) => !member.user.bot)
          .size,
        ...
        categories: guild.channels.cache.filter((channel) => channel.type === 4)
          .size,
        ...
        createdAt: guild.createdAt.toISOString(),
        botJoinedAt: guild.members.cache
          .get(discordClient.user?.id || "")
        ...
        ticketCategoriesCount:
          await dbManager.getTicketCategoriesCount(guildId),
      };
          ...
          try {
            ...
            return {
              ...
              targetName:
                ...
                `${permission.type === "role" ? "Rolle" : "Benutzer"} (${permission.target_id.substring(0, 8)}...)`,
              permissions: permission.permissions,
              createdAt: permission.created_at,
            };
          } catch (error) {
            console.error(
              `Error refreshing name for ${permission.type} ${permission.target_id}:`,
              error,
            );
            return {
              id: permission.id,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!guildId || !id) {
        return res
          .status(400)
          .json({ error: "Guild ID and Permission ID are required" });
      }
      ...
      if (parseInt(id) === -1) {
        return res
          .status(403)
          .json({ error: "Cannot remove server owner permissions" });
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (registerGuildCommandsFunction) {
        console.log(
          `🔄 Updating slash commands for guild ${guildId} due to feature changes...`,
        );
        try {
          await registerGuildCommandsFunction(guildId);
          console.log(
            `✅ Slash commands updated successfully for guild ${guildId}`,
          );
        } catch (commandError) {
          console.error(
            `❌ Failed to update commands for guild ${guildId}:`,
            commandError,
          );
        ...
        console.warn(
          "⚠️ registerGuildCommandsFunction not available - commands not updated",
        );
      }
...
app.get(
  "/api/settings/:guildId/features/:feature/status",
  requireAuth,
  requireGuildAccess,
  async (req: Request, res: Response) => {
    try {
      ...
      if (!guildId || !feature) {
        return res
          .status(400)
          .json({ error: "Guild ID and feature are required" });
      }
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `[DEBUG] Fetching members for guild ${guildId}, search: "${search}"`,
      );
      ...
      console.log(
        `[DEBUG] Guild found: ${guild.name}, cached members: ${guild.members.cache.size}/${guild.memberCount}`,
      );
      ...
      if (shouldFetchMembers) {
        console.log(
          `[DEBUG] Attempting to fetch members (server size: ${guild.memberCount})...`,
        );
        try {
      ...
      console.log(
        `[DEBUG] Total human members found in cache: ${members.length}`,
      );
      console.log(
        `[DEBUG] Member details:`,
        members.map(
          (m) => `${m.username}#${m.discriminator} (${m.displayName})`,
        ),
      );
      ...
      if (members.length === 0) {
        console.log(
          "[DEBUG] No human members in cache, providing fallback response",
        );
      ...
      if (search && typeof search === "string" && search.length >= 2) {
        ...
        console.log(
          `[DEBUG] Members after search filter "${search}": ${members.length}/${originalCount}`,
        );
      }
      ...
      console.log(
        `[DEBUG] Final member count to return: ${limitedMembers.length}`,
      );
...
app.get(
  "/api/tickets/:guildId/transcripts",
  requireAuth,
  requireGuildAccess,
  async (req: Request, res: Response) => {
    try {
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const transcript = await dbManager.getTicketTranscript(
        parseInt(ticketId),
        guildId,
      );
...
app.post("/api/tickets", requireAuth, async (req: Request, res: Response) => {
  try {
    const { userId, username, reason, guildId, channelId, categoryId } =
      req.body;
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!autoResponseId) {
        return res
          .status(409)
          .json({ error: "Auto response with this trigger already exists" });
      }
...
app.delete(
  "/api/autoresponses/:guildId/:trigger",
  requireAuth,
  requireGuildAccess,
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await dbManager.removeAutoResponse(
        decodedTrigger,
        guildId,
      );
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const categories = await dbManager.getTicketCategories(
        guildId,
        activeOnly,
      );
...
app.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await dbManager.updateTicketCategory(
        parseInt(categoryId),
        guildId,
        {
          name,
...
app.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await dbManager.deleteTicketCategory(
        parseInt(categoryId),
        guildId,
      );
...
app.get("/api/changelog/markdown", (req: Request, res: Response) => {
  try {
    const generateMarkdown = () => {
      ...
      markdown +=
        "All notable changes to BotTrapper will be documented in this file.\n\n";
