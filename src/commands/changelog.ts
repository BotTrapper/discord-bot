...
export const data = new SlashCommandBuilder()
  .setName('changelog')
  .setDescription('Zeigt Informationen über die aktuelle Version und Änderungen an')
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (requestedVersion) {
    ...
    const versionEntry = changelog.find(entry => entry.version === requestedVersion);
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0x5865F2)
      .setTimestamp()
      .setFooter({ text: 'BotTrapper', iconURL: interaction.client.user?.displayAvatarURL() });
    ...
    embed.addFields({ name: '🏷️ Typ', value: versionEntry.type.toUpperCase(), inline: true });
      ...
      if (items && items.length > 0) {
        ...
        if (value.length <= 1024) {
          ...
          chunks.forEach((chunk, index) => {
            embed.addFields({
              name: index === 0 ? `${emoji} ${label}` : `${emoji} ${label} (Fortsetzung)`,
              value: chunk,
              inline: false
            });
          });
  ...
  embed.addFields(
    ...
    { name: '📅 Gestartet', value: `<t:${Math.floor(versionInfo.startTime.getTime() / 1000)}:R>`, inline: true }
  );
    ...
    Object.entries(latestEntry.changes).forEach(([type, items]) => {
      if (items && items.length > 0) {
        const emoji = type === 'added' ? '✨' : 
                     type === 'changed' ? '🔄' : 
                     type === 'fixed' ? '🐛' : '🗑️';
        changesSummary += `${emoji} ${items.length} ${type === 'added' ? 'hinzugefügt' : 
                                                      type === 'changed' ? 'geändert' : 
                                                      type === 'fixed' ? 'behoben' : 'entfernt'}\n`;
      }
    });

    embed.addFields(
      { name: '📋 Letzte Änderungen', value: `**v${latestEntry.version}** (${date})\n${changesSummary}`, inline: false }
    );
  }

  embed.addFields({
    name: '🔗 Links',
    value: '• Dashboard für weitere Details\n• `/changelog <version>` für spezifische Versionen',
    inline: false
  });
