...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand(subcommand =>
    subcommand
      .setName('tickets')
      .setDescription('Zeige Ticket-Statistiken'))
  .addSubcommand(subcommand =>
    subcommand
      .setName('overview')
...
async function showCommandStats(interaction: any) {
  ...
  try {
    ...
    const commandList = stats.slice(0, 10).map((stat, index) => 
...
async function showTicketStats(interaction: any) {
  ...
  try {
    ...
    if (closedTickets.length > 0) {
      ...
      avgResolutionTime = totalResolutionTime / closedTickets.length / (1000 * 60 * 60); // in hours
    }

    const embed = EmbedBuilderFeature.createAdvancedEmbed({
      title: '🎫 Ticket Statistiken',
      fields: [
        { name: 'Gesamt Tickets', value: allTickets.length.toString(), inline: true },
        { name: 'Offene Tickets', value: openTickets.length.toString(), inline: true },
        { name: 'Geschlossene Tickets', value: closedTickets.length.toString(), inline: true },
        { name: 'Durchschnittliche Bearbeitungszeit', value: `${avgResolutionTime.toFixed(1)} Stunden`, inline: false }
      ],
      color: 0x00AE86
    });
...
async function showOverview(interaction: any) {
  ...
  try {
    ...
    const totalCommands = commandStats.reduce((sum, stat) => sum + stat.usage_count, 0);

    const embed = EmbedBuilderFeature.createAdvancedEmbed({
      title: '🤖 Bot Übersicht',
      fields: [
        { name: '🎫 Offene Tickets', value: openTickets.toString(), inline: true },
        { name: '🤖 Auto-Responses', value: responses.length.toString(), inline: true },
        { name: '🔗 Webhooks', value: webhooks.length.toString(), inline: true },
        { name: '📊 Commands (7 Tage)', value: totalCommands.toString(), inline: true },
        { name: '⚡ Status', value: 'Online', inline: true },
        { name: '📅 Letzter Neustart', value: new Date().toLocaleDateString('de-DE'), inline: true }
      ],
      color: 0x00AE86,
